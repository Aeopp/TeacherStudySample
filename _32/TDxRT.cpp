#include "TDxRT.h"
#include "TStd.h"
HRESULT TDxRT::Create(ID3D11Device* pd3dDevice, UINT w, UINT h)
{
	HRESULT hr = S_OK;
	m_TexDesc.Width = w;
	m_TexDesc.Height = h;
	...
	if (FAILED(hr = pd3dDevice->CreateRenderTargetView(
		m_pTexRT.Get(), NULL, m_pRTV.GetAddressOf())))
	{
		return hr;
	}
	if (FAILED(hr = pd3dDevice->CreateShaderResourceView(
		m_pTexRT.Get(), NULL, m_pRT_SRV.GetAddressOf())))
	{
		return hr;
	}
	...
	if (FAILED(hr = pd3dDevice->CreateDepthStencilView(
		m_pTexDS.Get(), NULL, m_pDSV.GetAddressOf())))
	{
		return hr;
	}
	...
	{
		if (FAILED(hr = pd3dDevice->CreateShaderResourceView(
			m_pTexDS.Get(), NULL, m_pDS_SRV.GetAddressOf())))
		{
			return hr;
		}
	}
...
bool TDxRT::Begin(ID3D11DeviceContext* pContext)
{
