...
ID3D11Buffer* DX::CreateVertexBuffer(ID3D11Device* pd3dDevice, void* pData, UINT iCounter, UINT iSize)
{
    HRESULT hr = S_OK;
...
ID3D11Buffer* DX::CreateIndexBuffer(ID3D11Device* pd3dDevice, void* pData, UINT iCounter, UINT iSize)
{
    HRESULT hr = S_OK;
    D3D11_BUFFER_DESC bd;
    ID3D11Buffer* pBuffer{ nullptr };
...
ID3D11Buffer* DX::CreateConstantBuffer(ID3D11Device* pd3dDevice, void* pData, UINT iCounter, UINT iSize)
{
    HRESULT hr;
    D3D11_BUFFER_DESC bd;
    ID3D11Buffer* pBuffer = nullptr;
...
ID3D11InputLayout* DX::CreateInputlayout(ID3D11Device* pd3dDevice, DWORD dwSize, LPCVOID lpData, D3D11_INPUT_ELEMENT_DESC* layout, UINT iElements)
{
    HRESULT hr = S_OK;
...
HRESULT DX::CompileShaderFromFile(ID3D11Device* pd3dDevice, const WCHAR* szFileName, DWORD flags, LPCSTR szEntryPoint, LPCSTR szShaderModel, ID3DBlob** ppBlobOut)
{
    HRESULT hr;
    ...
    hr = D3DCompile(pFileData,
        filesize.LowPart,
        "none",
        NULL, NULL, szEntryPoint, szShaderModel, flags,
...
ID3D11VertexShader* DX::LoadVertexShaderFile(ID3D11Device* pd3dDevice, const WCHAR* szFileName, DWORD flags, LPCSTR szEntryPoint, LPCSTR szShaderModel, ID3DBlob** ppBlobOut)
{
    HRESULT hr;
    ID3D11VertexShader* pVS = nullptr;
...
ID3D11PixelShader* DX::LoadPixelShaderFile(ID3D11Device* pd3dDevice, const WCHAR* szFileName, DWORD flags, LPCSTR szEntryPoint, LPCSTR szShaderModel, ID3DBlob** ppBlobOut)
{
    HRESULT hr;
...
bool DX::TDxObject::PreRender(ID3D11DeviceContext* pContext, UINT iVertexSize)
{
...
bool DX::TDxObject::Render(ID3D11DeviceContext* pContext, UINT iVertexSize, UINT iCount)
{
...
bool DX::TDxObject::PostRender(ID3D11DeviceContext* pContext, UINT iCount)
{
